	...
	class URLTarget(object):
		...
		def download(self):
			...
			try:
				...
				self.response_code = remote_file.getcode()
				if self.response_code == 200:
					self.size = int(remote_file.headers['content-length'])
		...
		def __str__(self):
			return '%(code)s, %(size)s, %(url)s, %(success)s, %(error)s' % {'code':self.response_code, 'size':self.size, 'url':self.url, 'success':self.success, 'error':self.error}
